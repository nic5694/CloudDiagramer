...
# @app.get('/projects/<pId>/generatepuml')
# def generate_puml(pId):
#     assert pId == request.view_args['pId']
...
def generate():
    ...
    if not data:
        ...
        # Optionally, enhance each asset (e.g., add notes)
    initial_json_clean = load_unfilteredJSON(json.dumps(data))
    for asset in initial_json_clean:
        asset['cohere'] = generate_notes_with_cohere(asset)
    ...
    link = f"https://www.plantuml.com/plantuml/png/{encode_plantuml(puml_output)}"
   
    return  {"diagram_url": link}, 200, {'Content-Type': 'text/plain'}
...
def generate_puml(pId):
    ...
    try:
        ...
        # Generate PUML code based on the list of assets.
        puml_output = generate_puml_from_assets(initial_json_clean)
        return {"diagram_url": puml_output}, 200, {'Content-Type': 'text/plain'}
